						...
						<div id="tours">
							...
							<div class="pageright" style="margin-left: 0; float: left;">
								...
								<div class="best-selling">
									<h2 style="margin-left: 10px; margin-top: 0px;text-align: center">Chi tiết đặt mua tour</h2>
									<!-- PAGE CONTENT -->
									<div class="greybox rounded" style="padding:10px; width:600px;">
										<div id="bookingInfo" style="padding: 15px 15px 15px 30px; float: left; width: 40%">
								   			Mã số: <span style="font-weight: bold; font-size: 18pt">${f:h(booking.bookingNumber) }</span><br/>
								   			Ngày mua: <fmt:formatDate value="${booking.ngayMua}" pattern="hh:mm dd-MM-yyyy" /><br/>
								   			Giá: ${f:h(tour.gia) } VNĐ<br/>
							   			</div>
							   			<div id="tourInfo" style="padding: 15px; float: left; width: 40%"}>
							   				Tour: <a style="color: #FC411C; font-size: large" href="/viewTour?tourKey=${f:h(tour.key) }">${f:h(tour.ten)}</a><br/>
						   					Tuyến đường: ${f:h(tour.ddXP)} >> ${f:h(tour.ddDich)}<br/>
						   					Ngày khởi hành: <fmt:formatDate value="${tour.ngayXP}" pattern="dd-MM-yyyy" /><br/>
						   					Thời gian: ${f:h(tour.thoiGian)} ngày<br/>
					   					</div>
					   					<div id="khachInfo" style="padding: 15px 15px 15px 30px; width: 40%; float: left;">
							   				
							   				Khách hàng: <a style="font-size: large; color: #2367AD" href="/viewProfile?profile=${f:h(khach.username)}">${f:h(khach.hoTen)}</a><br/>
							   				${f:h(khach.phone)}<br/>
							   				${f:h(khach.email)}<br/>
							   				${f:h(khach.diaChi)}<br/>
						   				</div>
						   				<div id="ctyInfo" style="padding: 15px; float: left; width: 40%">
							   				Công ty: <a style="font-size: large; color: #2367AD" href="/viewProfile?profle=${f:h(cty.username)}">${f:h(cty.hoTen)}</a><br/>
							   				${f:h(cty.phone)}<br/>
							   				${f:h(cty.email)}<br/>
							   				${f:h(cty.diaChi)}<br/>
							   			</div>
