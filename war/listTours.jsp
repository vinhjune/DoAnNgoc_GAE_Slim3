						...
						<div id="tours">
							...
							<div class="pageright" style="margin-left: 0; float: left;">
								...
								<div class="best-selling">
									<h2 style="font-size:22pt; margin-left: 10px; margin-top: 0px;text-align: center">Các tour hiện có</h2>
									<c:forEach var="t" items="${tours}">	<!-- ten, ctyName, moTa, thoiGian, gia, ngayXP, soLuong, ddXP, ddDich,--> 
		<div class="greybox rounded" style="padding:10px; width:600px;">
			<ul>
	   			<li>
   				
					<div style="" class="right">
						<a title="${f:h(t.ten)}" style="float:left" href="viewTour?tourKey=${f:h(t.key)}">
							<div class="bestsellingimgback">
								<img alt="${f:h(t.ten)}" src="${f:h(t.thumbnailUrl)}">
							</div>
						</a>
						<div class="featuretourbox white rounded">
							<h3>
								<a title="${f:h(t.ten)}" href="viewTour?tourKey=${f:h(t.key)}">${f:h(t.ten)}
								</a>
							</h3>
							<span class="gia rounded">${f:h(t.gia)} VNĐ</span>
							<span class="ngay rounded"><fmt:formatDate value="${t.ngayXP}" pattern="dd-MM-yyyy" /></span>
							<ul>
								
								<li><span><a href="/viewDiaDiem?dd=${f:h(t.ddXP)}">${f:h(t.ddXP)}</a></span> >> <span><a href="/viewDiaDiem?dd=${f:h(t.ddDich)}">${f:h(t.ddDich)}</a></span></li>
								
								<li><span><a href="/viewProfile?profile=${f:h(t.ctyName)}">${f:h(t.ctyName)}</a></span></li>
								<!-- 
								<li>
									<b>Xuat Phat:Ha Noi</b> <span>
									<a href="viewDiaDiem?dd=${f:h(t.ddXP)}">${f:h(t.ddXP)}</a>
									</span> - 
									<b>Den: Ho Chi Minh</b> 
									<span><a href="viewDiaDiem?dd=${f:h(t.ddDich)}">${f:h(t.ddDich)}</a></span>
								</li>
								-->
								<br/>
								<li><b>Thời gian:</b> <span>${f:h(t.thoiGian)} ngày.</span></li>
								<li><b>Số lượng:</b> <span>${f:h(t.soLuong)} chỗ.</span></li>
								<br/>
								<li><a style="float:none; font-style:italic; font-weight: bold; color:#FC411C" title="More info" class="more" href="viewTour?tourKey=${f:h(t.key)}">More info</a></li>
...
$(document).ready(function(){
	...
	<script type="text/javascript"
		src="http://scd.hwstatic.com/static/js//javascripts/translations.en.js"></script>
