						...
						<div id="tours">
							...
							<div class="pageright" style="margin-left: 0; float: left;">
								...
								<div class="best-selling">
									<h2 style="margin-left: 10px; margin-top: 0px;text-align: center">Giao dịch của tôi</h2>
									
										<div class="greybox rounded" style="padding:10px; width:600px;">
										<div style="padding:0px 20px">
										<table  cellpadding="12" cellspacing="10" border-spacing="-1" width="100%" style="border-collapse:separate;border-spacing: 0px 10px;">
										<tr>
											<th align="left" width="20%">Mã số</td>
											<th align="left" width="">Tour</td>
											<%if (userType.equals("khach")) {%>
											<th align="left" width="20%">Công ty</td> 
											<%} else if (userType.equals("congTy")){ %>
											<th align="left" width="20%">Khách</td>
											<%} %>
											<th align="left" width="13%">Ngày mua</td>
										</tr>
										<c:forEach var="b" items="${bookings}">	 
										<tr onmouseover="this.style.backgroundColor='#8CACFF';" onmouseout="this.style.backgroundColor='';">
											<td><a href="/viewBooking?bookingKey=${f:h(b.key) }">${f:h(b.bookingNumber)}</a></td>
											<td><a href="/viewTour?tourKey=${f:h(b.tourKey)}">${f:h(b.tourname) }</a></td>
											<%if (userType.equals("khach")) {%>
											<td width="20%"><a href="/viewProfile?profile=${f:h(b.ctyname)}">${f:h(b.ctyname)}</a></td>
											<%} else if (userType.equals("congTy")){ %>
											<td width="20%"><a href="/viewProfile?profile=${f:h(b.khachname) }">${f:h(b.khachname) }</a></td>
											<%} %>
											<td width="20%"><fmt:formatDate value="${b.ngayMua}" pattern="hh:mm dd-MM-yyyy" /></td>
										</tr>
										</c:forEach>	
										</table>
										</div>													
